...
class LightweightResidualBlock(nn.Module):
    """A simple lightweight residual block with improved shortcut connection."""
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # Improved shortcut connection
        if stride != 1 or in_channels != out_channels:
            self.shortcut = nn.Sequential(
                # Spatial projection using max pooling
                nn.MaxPool1d(kernel_size=stride, stride=stride),
                # Channel projection using 1x1 convolution
